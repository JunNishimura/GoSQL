...
// newCommitRecordPage lays out a commit record as it is stored in the log:
// the op code first, then the transaction number.
func newCommitRecordPage(t *testing.T, txNum int32) *filemanager.Page {
	t.Helper()
	p := filemanager.NewPageByBlockSize(commitRecordSize)
...
func TestNewCommitRecord(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			rec := NewCommitRecord(newCommitRecordPage(t, tt.txNum))
...
func TestCommitRecordUndo(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			rec := NewCommitRecord(newCommitRecordPage(t, tt.recordTxNum))
